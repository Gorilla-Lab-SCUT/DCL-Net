...
def init():
    ...
    cfg.objlist = [1, 2, 4, 5, 6, 8, 9, 10, 11, 12, 13, 14, 15]
    cfg.num_objects = 13
    for obj in cfg.objlist:
        diameter.append(meta[obj]['diameter'] / 1000.0 * 0.1)
    print(diameter)
    cfg.diameter = diameter
    tools_train.backup(["tools/test_LM.py"], log_dir)
...
def test(model, save_path, cfg, logger):
    ...
    dataset = DATASET.Dataset('eval', cfg.hyper_dataset_test, cfg.path_data)
...
if __name__ == "__main__":
    ...
    logger.warning("************************ Start Logging the Evaluation Process On LineMOD Dataset ************************")
    logger.info(cfg)
    ...
    DATASET = importlib.import_module(cfg.hyper_dataset_test.name[0])
    # model
