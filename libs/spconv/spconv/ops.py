...
def indice_fieldmaxpool(features, indice_pairs, indice_pair_num, num_activate_out, feature_norms):
    if features.dtype == torch.float32:
        return torch.ops.spconv.indice_field_maxpool_fp32(features, indice_pairs, indice_pair_num,
                                                  num_activate_out, feature_norms)
    elif features.dtype == torch.half:
        return torch.ops.spconv.indice_field_maxpool_half(features, indice_pairs, indice_pair_num,
                                                  num_activate_out, feature_norms)
    else:
        raise NotImplementedError
...
def get_indice_summaryrf(indice_pairs, indice_pair_num, num_activate_out):
    return torch.ops.spconv.indiceSummaryRF(indice_pairs, indice_pair_num, num_activate_out)

def indice_avgpool(features, indice_pairs, indice_pair_num, num_activate_out, summaryrf):
    if features.dtype == torch.float32:
        return torch.ops.spconv.indice_avgpool_fp32(features, indice_pairs, indice_pair_num,
                                                  num_activate_out, summaryrf)
    elif features.dtype == torch.half:
        return torch.ops.spconv.indice_avgpool_half(features, indice_pairs, indice_pair_num,
                                                  num_activate_out, summaryrf)
    else:
        raise NotImplementedError
def indice_avgpool_backward(features, out_features, out_bp, indice_pairs, indice_pair_num, summaryrf):
    if features.dtype == torch.float32:
        return torch.ops.spconv.indice_avgpool_backward_fp32(
            features, out_features, out_bp, indice_pairs, indice_pair_num, summaryrf)
    elif features.dtype == torch.half:
        return torch.ops.spconv.indice_avgpool_backward_half(
            features, out_features, out_bp, indice_pairs, indice_pair_num, summaryrf)
    else:
        raise NotImplementedError